...
Se observa que la mayoría de familias tienen entre 0 y 2 hijos, sin embargo, hay una proporción considerable de hogares que poseen 3, 4 y 5 hijos. A partir de los 6 hijos la proporción decae, por lo que se podría pensar en agrupar a los hogares en 5 categorias comprendidas como: 0, 1, 2, 3, 4, 5+.  
...
### Bosques Aleatorios para seleccion de variables

Se organiza la base de datos para poder aplicar el modelo
...
Con la función ranger aplicamos Bosques aleatorios
...
Utilizamos los pesos de importancia asignados por el Bosque Aleatorio para escoger las variables que se utilizaran en el resto de modelos pero ademas se tiene en cuenta la cantidad de NA de las variables.
...
Se debe convertir la variable respuesta nuevamente a numerica para poder aplicar regresión lineal.
...
La tasa de acierto es  `r acierto`, la tasa de error es `r error`
...
Hacemos las predicciones y obtenemos la matriz de confusión
...
Se entrena el modelo knn para que escoja el numero k optimo siendo el valor maximo k=30.
...
Con el k optimo se realiza el modelo se hacen las predicciones y se obtiene la matriz de confusión
