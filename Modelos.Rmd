...
MC <- table(test$Num_hijos, pred_mod1_round)
MC
# calculamos el error
acierto<-sum(diag(MC))/sum(MC)
acierto
...
```{r}
mod1_reg_arboles <- rpart(Num_hijos~., data = train)
```
...
pred_mod1_reg_arboles <-  predict(mod1_reg_arboles, newdata = test)
```
...
MC <- table(test$Num_hijos, pred_mod1_reg_arboles)
MC
# calculamos el error
acierto<-sum(diag(MC))/sum(MC)
acierto
...
```{r}
HijosPoisson <-glm(Num_hijos ~ ., family = poisson, data = train)
# Resumen del modelo
...
HijosPoisson <-  predict(HijosPoisson, newdata = test)
```
...
```{r}
plot(mod1_reg_arboles)
text(mod1_reg_arboles)
```
