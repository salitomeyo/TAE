...
```{r}
num_hijos_final <- merge(num_integrantes, num_hijos, by = "ï..DIRECTORIO", all.x=TRUE)

num_hijos_final <- num_hijos_final[,-4]
num_hijos_final$n.y[is.na(num_hijos_final$n.y)] <- 0
colnames(num_hijos_final) <- c("Directorio", "Secuencia", "Num Integrantes", "Num Hijos")
```


```{r}
head(num_hijos_final)
```
